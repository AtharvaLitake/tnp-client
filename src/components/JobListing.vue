        ...
        <v-card style="height:250px">
          <v-row class="border-b-sm">
            ...
            <v-col cols="8" class="d-flex flex-column justify-center text-left pl-3">
                <h1 class="text-h5 font-weight-bold text-primary">{{list.companyName}}</h1>
                <h1 class="text-h6 font-weight-bold text-primary">CTC - {{list.companyPackage}} LPA</h1>
            </v-col>
        </v-row>
            <div class="px-3 pt-4 pb-2 custom_colors text-subtitle-1 ma-0">
                <p>Role : {{ list.jobRole }}</p>
                <p>Job Location : {{ list.jobLocation}}</p>
                <p>Deadline: {{ new Date(list.applicationDeadline).toLocaleDateString('en-US', { day: 'numeric', month: 'short' }) }}</p>
            </div>
