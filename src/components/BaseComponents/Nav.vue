<template>
    <v-app-bar app fixed dense height="60" color="#081E7F" dark class="navbar" elevation="0" flat>
      ...
      <v-btn outlined color="white" to="/login" class="login-btn">LOGIN</v-btn></div>
  ...
  <v-btn icon>
    ...
    <v-expand-transition><!-- eslint-disable-line -->
      <v-sheet v-if="menuVisible" class="mobile-menu">
        <v-btn text to="/home" class="mobile-link">Home</v-btn>
        <v-btn text to="/about" class="mobile-link">About</v-btn>
        <v-btn text to="/jobs" class="mobile-link">Jobs</v-btn>
        <v-btn text to="/profile" class="mobile-link">Profile</v-btn>
      </v-sheet>
    </v-expand-transition>
  </template>
  ...
  .mobile-link {
    ...
    border: none; /* Ensures no border effect */
  }
